...
if [ ! -e  trypanosoma_annotated.txt ]; then
   $HOME/bin/datasets summary genome taxon Trypanosoma --annotated --as-json-lines | \
   $HOME/bin/dataformat tsv genome --fields \
...
genomas[7:8, ]
```
Comparando ambas filas, se observa que contienen el mismo ensamblaje pero en distinta versión o con alguna diferencia en el estado del ensamblado/fecha. Para comprobar si son duplicadas:
...
if [ ! -e ncbi_data/data/GCA_022059095.1/genomic.gff ]; then
   if [ ! -e ncbi_dataset.zip ]; then
      $HOME/bin/datasets download genome accession GCA_022059095.1 --include genome,gff3
   fi
...
```{r}
eliminar_invariables <- function(df) {
  variables <- sapply(df, function(x) length(unique(x)) > 1)
  df[, variables]
}

gene   <- eliminar_invariables(gene)
mrna   <- eliminar_invariables(mrna)
exon   <- eliminar_invariables(exon)
cds    <- eliminar_invariables(cds)
#region <- eliminar_invariables(region)
```
...
resumen <- data.frame(
   ...
   long.Media = c(mean(cds$longitud),
                  mean(gene$longitud),
                  mean(mrna$longitud),
                  mean(exon$longitud),
                  mean(region$longitud)),
   long.SD    = c(sd(cds$longitud),
                  sd(gene$longitud),
                  sd(mrna$longitud),
                  sd(exon$longitud),
                  sd(region$longitud))
