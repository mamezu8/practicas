...
estructuras <- fromJSON(paste0(API, 'structure'))
conjuntos <- fromJSON(paste0(API, 'set'))
proteomas <- fromJSON(paste0(API, 'proteome'))
taxonomias <- fromJSON(paste0(API, 'taxonomy'))
```
:::
...
```{r}
taxaTot <- fromJSON(paste0(API, 'taxonomy'))[['taxa']][['uniprot']]
taxaCon <- fromJSON(paste0(API, 'taxonomy/protein'))[['taxa']][['uniprot']]
taxaSin <- taxaTot - taxaCon

totProt <- fromJSON(paste0(API, 'protein'))[['proteins']][['uniprot']]
conStr <- fromJSON(paste0(API, 'protein/structure'))[['proteins']][['uniprot']]
sinStr <- totProt - conStr
sinStr
```
...
```{r}
pfam01 <- fromJSON(paste0(API, 'entry/pfam/taxonomy/uniprot/7742'))
class(pfam01)
length(pfam01)
names(pfam01)
pfam01[['count']]
...
```{r}
coco <- fromJSON(paste0(API, 'protein/entry/pfam/PF01530/taxonomy/uniprot/8493'))[['proteins']][['uniprot']]
```
:::
