...
echo "Lecturas:"
expr $(gunzip -c DRR025089_1.fastq.gz | wc -l) / 4
```
:::
...
Sí, debemos guardarlos en data porque representan datos que cogemos para realizar la práctica. 

- ¿Cómo podrías guardar los mensajes de `wget` en un archivo?
Los mensajes se podrían guardar con curl:

```{bash}
curl URL > salida.log 2> errores.log
```
...
| `-o`           | salida limpia R1  |
| `-O`           | salida limpia R2  |
| `--merge`      | fusionar lecturas |
| `--merged_out` | FASTQ fusionado   |

El programa fastp permite realizar control de calidad y filtrado de lecturas FASTQ. Las opciones usadas en esta práctica especifican los archivos de entrada y salida para las lecturas forward y reverse, además de activar la fusión de lecturas paired-end mediante --merge.
...
Las lecturas presentan buena calidad general, con valores altos de calidad Phred al inicio y una ligera disminución hacia el final de las lecturas, algo típico en secuenciación Illumina. fastp eliminó adaptadores y secuencias de baja calidad correctamente. El contenido GC es estable y compatible con un genoma bacteriano. Además, una proporción importante de lecturas paired-end pudo fusionarse, indicando buen solapamiento entre ellas.
