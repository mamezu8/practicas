...
El primero no funciona, pero el segundo sí porque tiene la fucnión URLencode. El tercero también funciona. 
...
1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción? 1258
2. ¿Qué longitudes tienen?  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
                             44     590     839    1252     967   17499 
3. ¿De cuántas especies diferentes son estas secuencias? 308
4. ¿Cuándo se publicaron las más recientes? "2022-04-03"
:::

```{r}
result <- 'result=sequence&'
query  <- 'query=tax_tree(7215) AND description="alcohol dehydrogenase"&'
fields <- 'fields=tax_id,scientific_name,description,last_updated,base_count,first_public&'
limit  <- 'limit=0'
URL <- paste0(portal, result, query, fields, limit)
curl_download(URLencode(URL), destfile = 'z2.txt')
```
...
platelmintos <- read.table(URLencode(URL),
    header = TRUE,
    colClasses = c('character', 'factor', 'character', 'character' , 'Date', 'numeric', 'Date'),
    sep = '\t',
    na.strings = '',
    quote= '')
```
